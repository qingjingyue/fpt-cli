...
public class SmsServiceImpl implements SmsService {
    ...
    public void sendPhoneVerifyCode(String phone) {
        ...
        SendSmsVerifyCodeRequest request = new SendSmsVerifyCodeRequest()
                ...
                .setTemplateParam("{\"code\":\"##code##\",\"min\":\"" + verifyCodeProperties.getTtl() + "\"}")// 短信模板参数, "##code##" 为阿里云服务生成的验证码
                .setCodeLength(6L)// 验证码长度，默认 4位
                .setValidTime(verifyCodeProperties.getTtl() * 60L)// 验证码有效期，默认 300 秒 (5分钟),单位秒
        ...
        try {
            ...
            log.info("发送短信验证码成功，手机号：{}，有效期：{}分钟", phone, verifyCodeProperties.getTtl());
        } catch (Exception e) {
    ...
    public void sendEmailVerifyCode(String email) {
        ...
        message.setText(String.format("您的验证码为%s。尊敬的客户，以上验证码%s分钟内有效，请注意保密，切勿告知他人。", code, verifyCodeProperties.getTtl()));  // 邮件内容
        // 发送邮箱验证码
        mailSender.send(message);
        log.info("发送邮箱验证码成功，邮箱：{}，验证码：{}，有效期：{}分钟", email, code, verifyCodeProperties.getTtl());
    }
